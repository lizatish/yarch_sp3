...
справляться с нагрузкой, он может быть отмасштабирован как на уровне приложени с равномерным распределением запросов
...
6) Сущность "Уведомление" - уведомление пользователю, включает в себя почту пользователя, текст сообщения, дату отправки
7) Сущность "Тип устройства" - перечисляемый тип, содержащий все поддерживаемые типы устройств на данный момент

# todo попросить Андрея проверить отношения на ER
...
- `token: str` - Токен для доступа к api. Можно получить обратившись по почте analiticexpertpay@gmail.com
...
2) TODO добавить `GET /api/v1/payments/{paymentId}`- Получить статус операции
...
- `device_id: uuid` - Идентификатор устройства
...
3) `PUT /api/v1/heating-sensor/{id}` - Дать команду на устройство
